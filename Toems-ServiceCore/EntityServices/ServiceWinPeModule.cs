...
{
    public class ServiceWinPeModule(ServiceContext ctx)
    {
        public DtoActionResult AddModule(EntityWinPeModule module)
        {
            ...
            if (validationResult.Success)
            {
                ...
                ctx.Uow.Save();
                ctx.Uow.WinPeModuleRepository.Insert(module);
                ctx.Uow.Save();
        ...
        public DtoActionResult DeleteModule(int moduleId)
        {
            ...
            var isActiveModule = ctx.Module.IsModuleActive(moduleId, EnumModule.ModuleType.WinPE);
        ...
        {
            return ctx.Uow.WinPeModuleRepository.GetById(moduleId);
        }

        public List<EntityWinPeModule> SearchModules(DtoSearchFilterCategories filter)
        {
            var list = ctx.Uow.WinPeModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && !s.Archived).OrderBy(x => x.Name).ToList();
        ...
        {
            return ctx.Uow.WinPeModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && s.Archived).OrderBy(x => x.Name).Take(filter.Limit).ToList();
        }

        public string TotalCount()
        {
            return ctx.Uow.WinPeModuleRepository.Count(x => !x.Archived);
        }
        ...
        public DtoActionResult UpdateModule(EntityWinPeModule module)
        {
            ...
            if (u == null) return new DtoActionResult {ErrorMessage = "Module Not Found", Id = 0};
            var isActiveModule = ctx.Module.IsModuleActive(module.Id, EnumModule.ModuleType.WinPE);
            if (!string.IsNullOrEmpty(isActiveModule)) return new DtoActionResult() { ErrorMessage = isActiveModule, Id = 0 };
            var validationResult = ValidateModule(module, false);
            var actionResult = new DtoActionResult();
            if (validationResult.Success)
            {
                ctx.Uow.WinPeModuleRepository.Update(module, module.Id);
                ctx.Uow.Save();
                actionResult.Success = true;
                actionResult.Id = module.Id;
            }
        ...
        private DtoValidationResult ValidateModule(EntityWinPeModule module, bool isNew)
        {
            ...
            if (isNew)
            {
                if (ctx.Uow.WinPeModuleRepository.Exists(h => h.Name == module.Name))
                {
                    validationResult.Success = false;
            ...
            {
                var originalModule = ctx.Uow.WinPeModuleRepository.GetById(module.Id);
