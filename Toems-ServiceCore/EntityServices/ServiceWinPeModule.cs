...
{
    public class ServiceWinPeModule(EntityContext ectx, ServiceModule moduleService)
    {
        public DtoActionResult AddModule(EntityWinPeModule module)
        {
            ...
            if (validationResult.Success)
            {
                ...
                ectx.Uow.Save();
                ectx.Uow.WinPeModuleRepository.Insert(module);
                ectx.Uow.Save();
        ...
        public DtoActionResult DeleteModule(int moduleId)
        {
            ...
            var isActiveModule = moduleService.IsModuleActive(moduleId, EnumModule.ModuleType.WinPE);
        ...
        {
            return ectx.Uow.WinPeModuleRepository.GetById(moduleId);
        }

        public List<EntityWinPeModule> SearchModules(DtoSearchFilterCategories filter)
        {
            var list = ectx.Uow.WinPeModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && !s.Archived).OrderBy(x => x.Name).ToList();
        ...
        {
            return ectx.Uow.WinPeModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && s.Archived).OrderBy(x => x.Name).Take(filter.Limit).ToList();
        }

        public string TotalCount()
        {
            return ectx.Uow.WinPeModuleRepository.Count(x => !x.Archived);
        }
        ...
        public DtoActionResult UpdateModule(EntityWinPeModule module)
        {
            ...
            if (u == null) return new DtoActionResult {ErrorMessage = "Module Not Found", Id = 0};
            var isActiveModule = moduleService.IsModuleActive(module.Id, EnumModule.ModuleType.WinPE);
            if (!string.IsNullOrEmpty(isActiveModule)) return new DtoActionResult() { ErrorMessage = isActiveModule, Id = 0 };
            var validationResult = ValidateModule(module, false);
            var actionResult = new DtoActionResult();
            if (validationResult.Success)
            {
                ectx.Uow.WinPeModuleRepository.Update(module, module.Id);
                ectx.Uow.Save();
                actionResult.Success = true;
                actionResult.Id = module.Id;
            }
        ...
        private DtoValidationResult ValidateModule(EntityWinPeModule module, bool isNew)
        {
            ...
            if (isNew)
            {
                if (ectx.Uow.WinPeModuleRepository.Exists(h => h.Name == module.Name))
                {
                    validationResult.Success = false;
            ...
            {
                var originalModule = ectx.Uow.WinPeModuleRepository.GetById(module.Id);
