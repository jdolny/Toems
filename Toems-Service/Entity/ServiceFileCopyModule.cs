...
{
    public class ServiceFileCopyModule
    {
        private readonly UnitOfWork _uow;

        public ServiceFileCopyModule()
        {
            _uow = new UnitOfWork();
        }
        ...
        public DtoActionResult AddModule(EntityFileCopyModule module)
        {
            ...
            if (validationResult.Success)
            {
                ...
                _uow.Save();
                _uow.FileCopyModuleRepository.Insert(module);
                _uow.Save();
        ...
        public DtoActionResult DeleteModule(int moduleId)
        {
            ...
            //_uow.FileCopyModuleRepository.Delete(moduleId);
            _uow.Save();
            var actionResult = new DtoActionResult();
        ...
        {
            return _uow.FileCopyModuleRepository.GetById(moduleId);
        }

        public List<EntityFileCopyModule> SearchModules(DtoSearchFilterCategories filter)
        {
            var list = _uow.FileCopyModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && !s.Archived).OrderBy(x => x.Name).ToList();
            ...
            var toRemove = new List<EntityFileCopyModule>();
        ...
        {
            return _uow.FileCopyModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && s.Archived).OrderBy(x => x.Name).Take(filter.Limit).ToList();
        }

        public string TotalCount()
        {
            return _uow.FileCopyModuleRepository.Count(x => !x.Archived);
        }
        ...
        public DtoActionResult UpdateModule(EntityFileCopyModule module)
        {
            ...
            if (validationResult.Success)
            {
                _uow.FileCopyModuleRepository.Update(module, module.Id);
                _uow.Save();
                actionResult.Success = true;
                actionResult.Id = module.Id;
            }
        ...
        private DtoValidationResult ValidateModule(EntityFileCopyModule module, bool isNew)
        {
            ...
            if (isNew)
            {
                if (_uow.FileCopyModuleRepository.Exists(h => h.Name == module.Name))
                {
                    validationResult.Success = false;
            ...
            {
                var originalModule = _uow.FileCopyModuleRepository.GetById(module.Id);
