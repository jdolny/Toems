...
{
    public class ServicePrinterModule
    {
        private readonly UnitOfWork _uow;

        public ServicePrinterModule()
        {
            _uow = new UnitOfWork();
        }
        ...
        public DtoActionResult DeleteModule(int moduleId)
        {
            ...
            var isActiveModule = new ServiceModule().IsModuleActive(moduleId, EnumModule.ModuleType.Printer);
            ...
            //_uow.PrinterModuleRepository.Delete(moduleId);
            _uow.Save();
            var actionResult = new DtoActionResult();
            actionResult.Success = true;
        ...
        {
            return _uow.PrinterModuleRepository.GetById(moduleId);
        }

        public List<EntityPrinterModule> SearchModules(DtoSearchFilterCategories filter)
        {
            var list = _uow.PrinterModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && !s.Archived).OrderBy(x => x.Name).ToList();
        ...
        {
            return _uow.PrinterModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && s.Archived).OrderBy(x => x.Name).Take(filter.Limit).ToList();
        }

        public string TotalCount()
        {
            return _uow.PrinterModuleRepository.Count(x => !x.Archived);
        }
        ...
        public DtoActionResult UpdateModule(EntityPrinterModule module)
        {
            ...
            if (u == null) return new DtoActionResult {ErrorMessage = "Module Not Found", Id = 0};
            var isActiveModule = new ServiceModule().IsModuleActive(module.Id, EnumModule.ModuleType.Printer);
            if (!string.IsNullOrEmpty(isActiveModule)) return new DtoActionResult() { ErrorMessage = isActiveModule, Id = 0 };
            var validationResult = ValidateModule(module, false);
            var actionResult = new DtoActionResult();
            if (validationResult.Success)
            {
                _uow.PrinterModuleRepository.Update(module, module.Id);
                _uow.Save();
                actionResult.Success = true;
                actionResult.Id = module.Id;
            }
        ...
        private DtoValidationResult ValidateModule(EntityPrinterModule module, bool isNew)
        {
            ...
            if (isNew)
            {
                if (_uow.PrinterModuleRepository.Exists(h => h.Name == module.Name))
                {
                    validationResult.Success = false;
            ...
            {
                var originalModule = _uow.PrinterModuleRepository.GetById(module.Id);
