...
{
    public class ServiceSysprepModule
    {
        private readonly UnitOfWork _uow;

        public ServiceSysprepModule()
        {
            _uow = new UnitOfWork();
        }
        ...
        public DtoActionResult DeleteModule(int moduleId)
        {
            ...
            var isActiveModule = new ServiceModule().IsModuleActive(moduleId, EnumModule.ModuleType.Sysprep);
        ...
        {
            return _uow.SysprepModuleRepository.GetById(moduleId);
        }

        public List<EntitySysprepModule> SearchModules(DtoSearchFilterCategories filter)
        {
            var list = _uow.SysprepModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && !s.Archived).OrderBy(x => x.Name).ToList();
        ...
        {
            return _uow.SysprepModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && s.Archived).OrderBy(x => x.Name).Take(filter.Limit).ToList();
        }

        public string TotalCount()
        {
            return _uow.SysprepModuleRepository.Count(x => !x.Archived);
        }
