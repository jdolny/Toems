...
{
    public class ServiceImagingClientId(ServiceContext ctx)
    {
        public DtoActionResult AddOrUpdate(int computerId)
        {
            ...
            var biosInfo = ctx.Uow.BiosInventoryRepository.Get(x => x.ComputerId == computerId).FirstOrDefault();
            if (biosInfo == null)
                return actionResult;
            if (string.IsNullOrEmpty(biosInfo.SerialNumber))
                return actionResult;

            var computerMacs = ctx.Uow.NicInventoryRepository.Get(x => x.ComputerId == computerId && x.Type.Equals("Ethernet")).Select(x => x.Mac).ToList();
            if (computerMacs.Count == 0)
                return actionResult;

            var pToRemove = ctx.Uow.ClientImagingIdRepository.Get(x => x.ComputerId == computerId);
            foreach (var mac in computerMacs)
            {
                ...
                var existing = ctx.Uow.ClientImagingIdRepository.GetFirstOrDefault(x => x.ComputerId == computer.Id && x.ClientIdentifier.Equals(clientImagingId));
                if (existing == null)
                {
                    ...
                    ctx.Uow.ClientImagingIdRepository.Insert(entityClientImagingId);
                }
                else
                {
                ...
                var existingSingleIdPretty = ctx.Uow.ComputerRepository.Get(x => x.ImagingClientId.Equals(clientImagingId) && x.ProvisionStatus == Toems_Common.Enum.EnumProvisionStatus.Status.ImageOnly).FirstOrDefault();
                if (existingSingleIdPretty != null)
                {
                    if (!LinkComputers(existingSingleIdPretty, computer))
                    {
                        //check raw
                        var existingSingleIdRaw = ctx.Uow.ComputerRepository.Get(x => x.ImagingClientId.Equals(rawIdentifier) && x.ProvisionStatus == Toems_Common.Enum.EnumProvisionStatus.Status.ImageOnly).FirstOrDefault();
            ...
            {
                ctx.Uow.ClientImagingIdRepository.Delete(p.Id);
            }
        ...
        {
            var provisionExists = ctx.Uow.ComputerRepository.Get(x => x.ImagingClientId.Equals(from.ImagingClientId) && x.ProvisionStatus == Toems_Common.Enum.EnumProvisionStatus.Status.Provisioned).FirstOrDefault();
            ...
            ctx.Uow.ComputerRepository.Update(to, to.Id);
            ctx.Uow.ComputerRepository.Delete(from.Id);
            ctx.Uow.Save();
            return true;
        }
