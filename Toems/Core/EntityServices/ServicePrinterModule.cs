...
{
    public class ServicePrinterModule(ServiceContext ctx)
    {
        ...
        {         
            ...
            {
                ctx.Uow.PrinterModuleRepository.Insert(module);
        ...
        public DtoActionResult DeleteModule(int moduleId)
        {
            ...
            var isActiveModule = ctx.Module.IsModuleActive(moduleId, EnumModule.ModuleType.Printer);
        ...
        {
            return ctx.Uow.PrinterModuleRepository.GetById(moduleId);
        }

        public List<EntityPrinterModule> SearchModules(DtoSearchFilterCategories filter)
        {
            var list = ctx.Uow.PrinterModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && !s.Archived).OrderBy(x => x.Name).ToList();
        ...
        {
            return ctx.Uow.PrinterModuleRepository.Get(s => (s.Name.Contains(filter.SearchText) || s.Guid.Contains(filter.SearchText)) && s.Archived).OrderBy(x => x.Name).Take(filter.Limit).ToList();
        }

        public string TotalCount()
        {
            return ctx.Uow.PrinterModuleRepository.Count(x => !x.Archived);
        }
        ...
        public DtoActionResult UpdateModule(EntityPrinterModule module)
        {
            ...
            if (u == null) return new DtoActionResult {ErrorMessage = "Module Not Found", Id = 0};
            var isActiveModule = ctx.Module.IsModuleActive(module.Id, EnumModule.ModuleType.Printer);
            if (!string.IsNullOrEmpty(isActiveModule)) return new DtoActionResult() { ErrorMessage = isActiveModule, Id = 0 };
            var validationResult = ValidateModule(module, false);
            var actionResult = new DtoActionResult();
            if (validationResult.Success)
            {
                ctx.Uow.PrinterModuleRepository.Update(module, module.Id);
                ctx.Uow.Save();
                actionResult.Success = true;
                actionResult.Id = module.Id;
            }
        ...
        private DtoValidationResult ValidateModule(EntityPrinterModule module, bool isNew)
        {
            ...
            if (isNew)
            {
                if (ctx.Uow.PrinterModuleRepository.Exists(h => h.Name == module.Name))
                {
                    validationResult.Success = false;
            ...
            {
                var originalModule = ctx.Uow.PrinterModuleRepository.GetById(module.Id);
