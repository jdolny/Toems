...
{
    public class SymmKeyHandler(ServiceContext ctx) : DelegatingHandler
    {
        ...
        protected override System.Threading.Tasks.Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, System.Threading.CancellationToken cancellationToken)
        {
            ...
            byte[] keyBytes = null;
            var computerEntity = ctx.Computer.GetByGuid(clientIdentifier);
            ...
            {
                var symmKey = ctx.Encryption.DecryptText(computerEntity.SymmKeyEncrypted);
        ...
        public StringContent ResponseHandler(Task<HttpResponseMessage> task, byte[] key,EntityComputer computer)
        {
            ...
            if(task.Result.Content != null)
            {
              ...
              var encryptedContent = new ServiceSymmetricEncryption().EncryptData(key, body);
                var signature = ctx.Certificate.SignMessage(Convert.ToBase64String(encryptedContent),
                    computer);
