...
namespace Toems_FrontEnd.views.modules.commandmodules
{
    public partial class archived : BasePages.Modules
    {
        ...
        protected void PopulateGrid()
        {
            ...
            var listOfModules = Call.CommandModuleApi.GetArchived(filter);
            gvModules.DataSource = listOfModules;
            gvModules.DataBind();

            lblTotal.Text = gvModules.Rows.Count + " Result(s) / " + Call.CommandModuleApi.GetArchivedCount() + " Module(s)";
        }
            ...
            switch (action)
            {
                case "delete":
                    foreach (GridViewRow row in gvModules.Rows)
                    {
                        ...
                        if (Call.CommandModuleApi.Delete(Convert.ToInt32(dataKey.Value)).Success)
                            count++;
                    }
                ...
                case "archive":
                    foreach (GridViewRow row in gvModules.Rows)
                    {
                        ...
                        var dataKey = gvModules.DataKeys[row.RowIndex];
                        if (dataKey == null) continue;
                        if (Call.ModuleApi.Archive(Convert.ToInt32(dataKey.Value), EnumModule.ModuleType.Command).Success)
                            count++;
                    }
        ...
        protected void btnRestore_OnClick(object sender, EventArgs e)
        {
            if (sender is Control control)
            {
                ...
                if (dataKey != null)
                {
                    var result = Call.ModuleApi.Restore(Convert.ToInt32(dataKey.Value),EnumModule.ModuleType.Command);
