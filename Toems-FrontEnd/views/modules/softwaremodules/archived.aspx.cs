...
namespace Toems_FrontEnd.views.modules.softwaremodules
{
    public partial class archived : BasePages.Modules
    {
        ...
        protected void PopulateGrid()
        {
            ...
            var listOfModules = Call.SoftwareModuleApi.GetArchived(filter);
            gvModules.DataSource = listOfModules;
            gvModules.DataBind();

            lblTotal.Text = gvModules.Rows.Count + " Result(s) / " + Call.SoftwareModuleApi.GetArchivedCount() + " Module(s)";
        }
        ...
        protected void gridView_Sorting(object sender, GridViewSortEventArgs e)
        {
            ...
            var listModules = (List<EntitySoftwareModule>)gvModules.DataSource;
            switch (e.SortExpression)
            {
                ...
                case "InstallType":
                    listModules = GetSortDirection(e.SortExpression) == "Desc"
                        ? listModules.OrderByDescending(h => h.InstallType).ToList()
                        : listModules.OrderBy(h => h.InstallType).ToList();
                    break;
            ...
            switch (action)
            {
                case "delete":
                    foreach (GridViewRow row in gvModules.Rows)
                    {
                        ...
                        if (Call.SoftwareModuleApi.Delete(Convert.ToInt32(dataKey.Value)).Success)
                            count++;
                    }
                ...
                case "archive":
                    foreach (GridViewRow row in gvModules.Rows)
                    {
                        ...
                        var dataKey = gvModules.DataKeys[row.RowIndex];
                        if (dataKey == null) continue;
                        if (Call.ModuleApi.Archive(Convert.ToInt32(dataKey.Value), EnumModule.ModuleType.Software).Success)
                            count++;
                    }
        ...
        protected void btnRestore_OnClick(object sender, EventArgs e)
        {
            if (sender is Control control)
            {
                ...
                if (dataKey != null)
                {
                    var result = Call.ModuleApi.Restore(Convert.ToInt32(dataKey.Value), EnumModule.ModuleType.Software);
