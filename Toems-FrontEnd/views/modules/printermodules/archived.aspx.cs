...
namespace Toems_FrontEnd.views.modules.printermodules
{
    public partial class archived : BasePages.Modules
    {
        ...
        protected void PopulateGrid()
        {
            ...
            var listOfModules = Call.PrinterModuleApi.GetArchived(filter);
            gvModules.DataSource = listOfModules;
            gvModules.DataBind();

            lblTotal.Text = gvModules.Rows.Count + " Result(s) / " + Call.PrinterModuleApi.GetArchivedCount() + " Module(s)";
        }
        ...
        protected void gridView_Sorting(object sender, GridViewSortEventArgs e)
        {
            ...
            var listModules = (List<EntityPrinterModule>)gvModules.DataSource;
            switch (e.SortExpression)
            {
                ...
                case "NetworkPath":
                    listModules = GetSortDirection(e.SortExpression) == "Desc"
                        ? listModules.OrderByDescending(h => h.NetworkPath).ToList()
                        : listModules.OrderBy(h => h.NetworkPath).ToList();
                    break;
                case "Action":
                    listModules = GetSortDirection(e.SortExpression) == "Desc"
                        ? listModules.OrderByDescending(h => h.Action).ToList()
                        : listModules.OrderBy(h => h.Action).ToList();
                    break;
                case "IsDefault":
                    listModules = GetSortDirection(e.SortExpression) == "Desc"
                        ? listModules.OrderByDescending(h => h.IsDefault).ToList()
                        : listModules.OrderBy(h => h.IsDefault).ToList();
                    break;
            ...
            switch (action)
            {
                case "delete":
                    foreach (GridViewRow row in gvModules.Rows)
                    {
                        ...
                        if (Call.PrinterModuleApi.Delete(Convert.ToInt32(dataKey.Value)).Success)
                            count++;
                    }
                ...
                case "archive":
                    foreach (GridViewRow row in gvModules.Rows)
                    {
                        ...
                        var dataKey = gvModules.DataKeys[row.RowIndex];
                        if (dataKey == null) continue;
                        if (Call.ModuleApi.Archive(Convert.ToInt32(dataKey.Value), EnumModule.ModuleType.Printer).Success)
                            count++;
                    }
        ...
        protected void btnRestore_OnClick(object sender, EventArgs e)
        {
            if (sender is Control control)
            {
                ...
                if (dataKey != null)
                {
                    var result = Call.ModuleApi.Restore(Convert.ToInt32(dataKey.Value), EnumModule.ModuleType.Printer);
