...
{
    public partial class resetrequests : BasePages.Computers
    {
        ...
        protected void PopulateGrid()
        {
            ...
            var listOfComputers = Call.ResetRequestApi.Search(filter);
            gvComputers.DataSource = listOfComputers;
            gvComputers.DataBind();

            lblTotal.Text = gvComputers.Rows.Count + " Result(s) / " + Call.ResetRequestApi.GetCount() + " Reset Request(s)";
        }
        ...
        protected void gridView_Sorting(object sender, GridViewSortEventArgs e)
        {
            ...
            var listModules = (List<EntityResetRequest>)gvComputers.DataSource;
            ...
            switch (action)
            {
                case "deny":
                    foreach (GridViewRow row in gvComputers.Rows)
                    {
                        ...
                        if (Call.ResetRequestApi.Delete(Convert.ToInt32(dataKey.Value)).Success)
                            count++;
                    }
                ...
                case "approve":
                    foreach (GridViewRow row in gvComputers.Rows)
                    {
                        ...
                        var dataKey = gvComputers.DataKeys[row.RowIndex];
                        if (dataKey == null) continue;
                        if (Call.ResetRequestApi.Approve(Convert.ToInt32(dataKey.Value)).Success)
                            count++;
                    }
                    EndUserMessage = "Approved " + count + " Reset Request(s)";
                    break;
            }
            PopulateGrid();
        }
        ...
        {
            lblTitle.Text = "Approve Selected Reset Requests?";
        ...
        {
            lblTitle.Text = "Deny Selected Reset Requests?";
            Session["action"] = "deny";
            DisplayConfirm();
        }
    }
